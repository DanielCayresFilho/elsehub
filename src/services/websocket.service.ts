...
class WebSocketService {
  ...
  async connect() {
    ...
    if (!accessToken) {
      logger.warn('[WebSocket] Token não disponível, não é possível conectar')
      return
    }

    try {
      logger.log('[WebSocket] Conectando...', this.wsUrl)
      ...
      this.socket.on('connect', () => {
        logger.log('[WebSocket] Conectado com sucesso')
        // Reentra em todas as salas que estava antes da desconexão
        this.joinedRooms.forEach(roomId => {
          logger.log('[WebSocket] Reentrando na sala:', roomId)
      ...
      this.socket.on('connect_error', (error) => {
        logger.error('[WebSocket] Erro de conexão', error)
      })

      // Escuta eventos de novas mensagens (conforme documentação: message:new)
      this.socket.on('message:new', (message: Message) => {
        logger.log('[WebSocket] Nova mensagem recebida:', message)
      ...
      this.socket.on('conversation:closed', (data: { conversationId: string } | Conversation) => {
        logger.log('[WebSocket] Conversa fechada:', data)
      ...
      this.socket.on('typing:user', (data: { userId: string; email: string; isTyping: boolean }) => {
        logger.log('[WebSocket] Usuário digitando:', data)
  ...
  joinRoom(conversationId: string) {
    if (!this.socket?.connected) {
      logger.warn('[WebSocket] Não conectado, não é possível entrar na sala')
      // Adiciona à lista para entrar quando reconectar
      this.joinedRooms.add(conversationId)
      return
    }
    logger.log('[WebSocket] Entrando na sala:', conversationId)
  ...
  leaveRoom(conversationId: string) {
    ...
    logger.log('[WebSocket] Saindo da sala:', conversationId)
  ...
  private emit(event: string, data: any) {
    this.listeners.get(event)?.forEach(callback => {
      try {
        ...
        logger.error(`[WebSocket] Erro ao executar listener para ${event}`, error)
      }
    })
  }
  ...
  disconnect() {
    if (this.socket) {
      logger.log('[WebSocket] Desconectando...')
